...
// Проверка и создание директории (создание рекурсивно если нужно)
...
function parseMultipart(body, boundary) {
    ...
    for (const part of parts) {
        // Пропуск пустых частей, завершающих границ
        if (part === '--' || part === '' || part === '--\r\n') continue;
...
const server = http.createServer((req, res) => {
    ...
    // Обработка POST запроса для загрузки файлов
        ...
        if (!boundaryMatch) {
            // Отправка ошибки если boundary не найден
        ...
        req.on('end', () => {
            // Получение границы из регулярного выражения
            ...
            if (parsed.file && parsed.file.data) {
                ...
                // Отправка успешного ответа с информацией о файле
                ...
                // Если файл не найден, отправка всех парсированных данных
